...
**다음** — 5C-4 아이 화면 (자유 모드 2단계, 새 단원 알림) → 6A 단답형
...
node test/t_units.mjs      # 46개 — 과목·단원·진도. 부모 화면 import가 안 깨졌는지
...
> `기획서.md` 16.6·16.8과 `구현-현황.md` 21.7을 읽고, 5C-4를 만들어줘.
> 자유 모드를 과목 → 단원 두 단계로, 그리고 새 단원이 열리면 홈에 알림이야.
> **안 열린 단원을 회색으로 보여주지 마** — 캐릭터 실루엣과 달리 학교가 여는 것이라
> 목표가 아니라 압박이 돼(16.5).
