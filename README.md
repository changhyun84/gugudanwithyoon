...
- CSV의 `난이도` 칸에 `심화`를 적으면 **그 단원의 기본을 지난 뒤에** 나옴 (기획서 12.0)
...
- **"같은 문제만 나온다"는 말이 나올 때** → 짐작하지 말고 `node sim/repeat.mjs`. 가장 큰 손잡이는 코드가 아니라 **부모가 켠 단원 수**입니다 (기획서 11.5)
...
node sim/repeat.mjs        # 같은 문제가 얼마나 반복되는지. "또 이 문제" 소리가 나오면 이것부터
```
