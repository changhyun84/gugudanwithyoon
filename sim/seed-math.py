"""수학 문제 팩 만들기 — python3 sim/seed-math.py [--force]
   ...
   **손으로 쓰지 않고 계산해서 만듭니다.** 559문제를 손으로 적으면 반드시 계산 실수가 나고,
...
def dedupe(rows):
    """같은 문제 글이 두 번 들어오면 키가 겹친다 (팩id:문제글). **먼저 온 것을 남긴다** —
       뒤에 붙이는 문제가 이미 있던 문제를 밀어내면 아이 진도가 그만큼 사라진다."""
    seen, out = set(), []
    for r in rows:
        if r[0] in seen:
            continue
...
# 구구단에서 진짜 외워야 하는 것들 (engine.js TARGETS와 같은 뜻).
# 2·5·9단은 규칙이 있어 쉬우므로 드릴에서는 뒤로 돌린다.
HARD = [(3,4),(3,6),(3,7),(3,8),(4,4),(4,6),(4,7),(4,8),(6,6),(6,7),(6,8),(7,7),(7,8),(8,8)]
EASYISH = [(3,3),(4,5),(5,6),(5,7),(5,8),(6,9),(7,9),(8,9),(3,9),(4,9)]
...
def div_basic():
    ...
    # 빈칸 곱셈은 사실 나눗셈이다 — 곱셈과 나눗셈을 잇는 가장 짧은 다리다.
    # 앞자리·뒷자리를 **둘 다** 비운다. 한쪽만 비우면 자리를 외워버린다.
    for a, b in HARD + EASYISH:
        rows.append(q(f'{a} × ? = {a*b}', b, f'{a*b}을(를) {a}(으)로 나누면 돼', '곱셈과 나눗셈'))
        rows.append(q(f'? × {b} = {a*b}', a, f'{a*b}을(를) {b}(으)로 나누면 돼', '곱셈과 나눗셈'))
    for a, b in HARD:
        rows.append(q(f'{a*b} ÷ {b}', a, f'곱셈으로 바꿔봐. ? × {b} = {a*b}', '나눗셈'))
    return dedupe(rows)
...
def mul_2x1():
    ...
    # 빈칸 곱셈 — 두 자리라도 답은 한 자리라 구구단으로 되짚을 수 있다
    for a, b in [(12,3),(21,4),(23,2),(32,3),(24,2),(13,3),(31,3),(22,4),(41,2),(11,7),
                 (17,3),(26,4),(38,2),(45,3),(27,6),(19,5),(36,6),(47,4),(52,8),(64,3)]:
        rows.append(q(f'{a} × ? = {a*b}', b, f'{a}이(가) 몇 번 모이면 {a*b}이(가) 될까?', '빈칸 곱셈'))
    return dedupe(rows)
...
def div_remainder():
    ...
    # 딱 떨어지는 나눗셈의 빈칸. 나머지를 배우기 전에 «몇 번 들어가나»가 몸에 붙어야 한다
    for a, b in [(72,8),(84,7),(96,6),(78,6),(85,5),(51,3),(68,4),(92,4),(87,3),(76,4),
                 (91,7),(98,7),(81,9),(90,6),(64,4)]:
        rows.append(q(f'{a} ÷ ? = {a//b} 일 때 ?는 얼마일까?', b,
                      f'{a//b}이(가) 몇 번 모이면 {a}이(가) 될까?', '빈칸 나눗셈'))
    return dedupe(rows)
...
def deep_mul_2x1():
    ...
    for a, b in [(3, 72), (4, 96), (5, 85), (6, 78), (7, 91),
                 (3, 51), (4, 68), (5, 95), (6, 84), (7, 84),
                 (8, 96), (9, 72), (4, 92), (6, 96), (8, 88)]:
...
def deep_mul_big():
    ...
    for a, b in [(4, 268), (3, 219), (6, 342), (5, 285), (7, 371),
                 (2, 156), (3, 261), (4, 328), (5, 415), (6, 174),
                 (7, 259), (8, 344), (9, 261), (4, 296), (6, 258)]:
        rows.append(d(f'? × {a} = {b} 일 때 ?는 얼마일까?', b // a, f'{b}을(를) {a}(으)로 나누면 돼', '거꾸로 곱셈'))
    # 두 자리 × 두 자리의 빈칸. 앞자리를 비우면 «몇 십 몇»을 통째로 떠올려야 한다
    for a, b in [(12,13),(21,14),(23,12),(31,23),(15,22),(24,31),(33,12),(16,21),(25,13),(42,11)]:
        rows.append(d(f'{a} × ? = {a*b} 일 때 ?는 얼마일까?', b,
                      f'{a*b}을(를) {a}(으)로 나누면 돼', '거꾸로 곱셈'))
    return dedupe(rows)
...
def deep_div_remainder():
    ...
    # 두 자리로 나누기 — 교과서로는 4학년 1학기다. 여기에 둔 이유는 **나눗셈을 곱셈으로
    # 되돌리는 것**이 이 아이에게 지금 필요한 바로 그 동작이기 때문이다. 힌트도 그렇게 준다.
    # 어렵다 싶으면 부모 화면에서 이 묶음을 꺼도 나머지 45문제는 그대로 남는다.
    for a, b in [(108,12),(96,12),(84,12),(72,12),(91,13),(98,14),(105,15),(112,16),
                 (102,17),(126,18),(114,19),(120,20),(132,22),(150,25),(168,24)]:
        rows.append(d(f'{a} ÷ {b}', a // b, f'곱셈으로 바꿔봐. {b} × ? = {a}', '두 자리로 나누기'))
    return dedupe(rows)
